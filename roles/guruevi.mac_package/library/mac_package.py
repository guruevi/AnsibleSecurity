...
class Archive(object):
    ...
    def acquire(self):
        ...
        if exists(self.params, 'src'):
            ...
            # get pkg from url wget --no-cookies --no-check-certificate --directory-prefix=${WORK_DIR} -c --header
            # "Cookie: $COOKIE" ${URL} wget --no-cookies --no-check-certificate --directory-prefix=${WORK_DIR} -c ${
            # URL} curl --location --insecure --cookie gpw_e24=http%3A%2F%2Fwww.oracle.com TODO: verify url
            url = self.params['url']
